...
namespace MenShopBlazor.Services.Storage
{
    public class StorageService : IStorageService
    {
        ...
        private const string baseUrl = "https://localhost:52221/api/Storage";
        ...
        public async Task<List<StorageDTO>?> GetAllProductsAsync()
        {
            try
            {
                var response = await HttpHelper.SendRequestAsync<List<StorageDTO>>(() => _httpClient.GetAsync($"{baseUrl}"));
        ...
        public async Task<List<ProductDetailViewModel>> GetByProductDetailId(int productId)
        {
            try
            {
